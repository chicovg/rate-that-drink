...
The `home-routes` handler in the `the-beer-tasting-app.routes.home` namespace
...
The `home-page` function will in turn call the `the-beer-tasting-app.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `the-beer-tasting-app.handler` namespace:
...
Request middleware functions are located under the `the-beer-tasting-app.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `the-beer-tasting-app.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `the-beer-tasting-app.db.core` in some other namespace.
